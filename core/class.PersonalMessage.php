...
class PersonalMessage extends User {
	...
	public function getPM($id = '', $nickname = '') {
		if($id !== '')
			if($this->isPM($id))
				...
				return false;
		elseif($nickname !== '')
			return ($result = parent::get("SELECT * FROM {$this->prefix}personalmessage WHERE receiver='$nickname' ORDER BY id")) ? $result : false;
		else
			return false;
	}
	...
	public function countPM() {
		...
		return parent::resultCountQuery("SELECT COUNT(*) FROM {$this->prefix}personalmessage WHERE receiver='$nickname' AND readed='0'");
	}
	...
	public function createPM($array) {
		...
		if((parent::isUser($array[0])) && (parent::isUser($array[1]))) {
			$query = 'INSERT INTO '.$this->prefix.'personalmessage(sender, receiver, date, hour, subject, message, readed) VALUES(';
	...
	public function readedPM($id) {
		return parent::query("UPDATE {$this->prefix}personalmessage SET readed='1' WHERE id='$id'") ? true : false;
	}
