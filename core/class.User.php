...
class User extends Configuration {
	...
	public $salt = 'ujihkuijbkugbhgvuyvg';
	...
	public function createUser($array) {
		...
		if((!$this->isUser($array[0])) && (parent::isEmail($array[2]))) {
			...
			$query = 'INSERT INTO '.$this->prefix.'utenti(nickname, password, email, grado, data, ora, codiceregistrazione, storage, profile, language) VALUES(';
	...
	/* Logga un utente. */
	public function login($nickname, $password, $language) {
		if(!$this->isUser($nickname))
			return false;
		...
		if(parent::resultCountQuery("SELECT COUNT(*) FROM {$this->prefix}utenti WHERE nickname='$nickname' AND password='$password' AND codiceregistrazione=''") > 0) {
			...
			parent::query("UPDATE {$this->prefix}utenti SET secret='$code', ip='{$_SERVER['REMOTE_ADDR']}', browsername='{$client['browser']}', browserversion='{$client['version']}', platform='{$client['platform']}', language='$language' WHERE nickname='$nickname'");
			$this->setCookie($code);
			return true;
		}
		else
